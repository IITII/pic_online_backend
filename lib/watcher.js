...
const cluster = require('cluster'), path = require('path'), chokidar = require('chokidar'),
  ...
  watcher = chokidar.watch(config.pic_dir, config.chokidar), {debounce} = require('../lib/utils')
let version = 1;
...
const listener = () => {
  const tmp = dirDetail(config.base_dir, config.pic_dir, config.prefix, config.includeRegex)
  process.send({
    version: version++,
    fileInfo: tmp
  })
  ...
  if (message.hasOwnProperty('command')) {
    switch (message.command) {
      case "refresh":
        ...
        let tmp = dirDetail(config.base_dir, config.pic_dir, config.prefix, config.includeRegex)
        process.send({
          version: version++,
          fileInfo: tmp
        })
